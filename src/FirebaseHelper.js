...
export default class FirebaseHelper {
  ...
  /**
   * Uploads a new video to Cloud Storage and adds a new post referencing it.
   * This returns a Promise which completes with the new Post ID.
   */
  uploadNewVid(video, fileName, text) {
    // Get a reference to where the post will be created.
    const newPostKey = this.database.ref('/posts').push().key;

    // Start the video file upload to Cloud Storage.
    const videoRef = this.storage.ref(`${this.auth.currentUser.uid}/full/${newPostKey}/${fileName}`);
    const metadata = {
      contentType: video.type,
    };
    const vidUploadTask = videoRef.put(video, metadata).then((snapshot) => {
      console.log('New video uploaded. Size:', snapshot.totalBytes, 'bytes.');
      return snapshot.ref.getDownloadURL().then((url) => {
        console.log('File available at', url);
        return url;
      });
    }).catch((error) => {
      console.error('Error while uploading new video', error);
    });


    return Promise.all([vidUploadTask]).then((urls) => {
      // Once video has been uploaded add a new post in the Firebase Database and
      // to its fanned out posts lists (user's posts and home post).
      const update = {};
      update[`/posts/${newPostKey}`] = {
        full_url: urls[0],
        ...
        client: 'web',
        timestamp: firebase.database.ServerValue.TIMESTAMP,
        full_storage_uri: videoRef.toString(),
        author: {
          uid: this.auth.currentUser.uid,
          full_name: this.auth.currentUser.displayName || 'Anonymous',
          profile_picture: this.auth.currentUser.photoURL || null,
