...
/*
    ...
    if (file.type.match('video.*')) {
      uploadNewVid(file, file.name, imageCaption)
    }
    */
  }
  ...
  generateImages() {
    ...
    const displayPicture = (url) => {
      ...
      // const fullCanvas = Uploader._getScaledCanvas(video, maxFullDimension);
      fullCanvas.toBlob(resolveFullBlob, 'image/jpeg', Uploader.FULL_IMAGE_SPECS.quality);
      // fullCanvas.toBlob(resolveFullBlob, 'video/*', Uploader.FULL_IMAGE_SPECS.quality);
    };
  ...
  uploadPic(e) {
    ...
    const imageCaption = this.imageCaptionInput.val();
    console.log(this.currentFile.type.match('video.*'));

    // upload a file if its of type video
    if (this.currentFile.type.match('video.*')) {
      this.firebaseHelper.uploadNewVid(this.currentFile, this.currentFile.name, imageCaption)
        .then((postId) => {
        page(`/user/${this.auth.currentUser.uid}`);
        const data = {
          message: 'New video has been posted!',
        ...
        console.error(error);
        const data = {
          message: `There was an error while posting your video. Sorry!`,
          timeout: 5000,
        };
        this.toast[0].MaterialSnackbar.showSnackbar(data);
        this.disableUploadUi(false);
    ...
    // upload a file if its of type image
