...
export default class Post {
  ...
  /**
   * enters the theatre mode.
   */
  enterTheatreMode(picUrl) {
    const url = require('url');
    const Path = require('path');
    const result = Path.extname(url.parse(picUrl).pathname); // '.jpg'
    const options = ['.mov', '.mp4', '.avi', '.flv', '.wmv'];
  ...
  _setupThumb(thumbUrl, picUrl) {
    ...
    const url = require('url');
    const Path = require('path');
    const result = Path.extname(url.parse(thumbUrl).pathname); // '.jpg'
    const options = ['.mov', '.mp4', '.avi', '.flv', '.wmv'];

    if (options.indexOf(result) > -1) {
      const res = result.split('.').pop(-1);
      $('.fp-image', post).html('<video id="my-video" class="video-js" controls preload="auto" width="100%" height="100%" data-setup="{}"> <source src="'+thumbUrl+'" type="video/'+res+'"> <p class="vjs-no-js">To view this video please enable JavaScript, and consider upgrading to a web browser that <a href="https://videojs.com/html5-video-support/" target="_blank">supports HTML5 video</a> </p> </video>');
    } else if (options.indexOf(result) < 0) {
